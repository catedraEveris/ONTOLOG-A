...
with open('Cluster_noRepeat.txt', 'rt') as f:
...
#Ahora haremos la comparacion en bucle buscando elementos
    ...
    for i in range(len(engtesta)):
        if engtesta[i][3] != "O" and engtesta[i][0] != "\n":
            for j in range(len(clusters)):
                for k in range(len(clusters[j])):
                    if engtesta[i][3].split('-')[1] == tags_clus[j]: 
                        if engtesta[i][0].lower() == clusters[j][k].lower():
                            #print("Nueva palabra encontrada: ", engtesta[k][0], " Tag: ", tags_clus[i], "-", tags1_clus[i])
                            engtesta[i][3] = "I-"+tags_clus[j]+"-"+tags1_clus[j]
                            break
